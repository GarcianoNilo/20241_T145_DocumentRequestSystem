...
function DisplayUser() {
    ...
    const columns = [
        ...
        { name: 'Department', selector: (row) => row.department, sortable: true },
        {
            ...
            cell: (row) => (
                <div>
                    <button className='button edit-user' onClick={() => handleEditUser(row)}>Edit</button>
                    <button className='button delete-user' onClick={() => handleDeleteUser(row.userID)}>Delete</button>
                </div>
            ),
        },
    ];
    ...
    return (
        <div style={{ padding: '20px' }}>
            ...
            {/* Add User Button */}
            <button className='button add-user' onClick={handleAddUser}>Add User</button>
            ...
            {isEditModalOpen && (
                <Modal isOpen={isEditModalOpen} onRequestClose={() => setIsEditModalOpen(false)}>
                    ...
                    />
                    <button className='button confirm-user' onClick={confirmEditUser}>Confirm</button>
                    <button className='button cancel-user' onClick={() => setIsEditModalOpen(false)}>Cancel</button>
                </Modal>
            )}
            ...
            {isAddModalOpen && (
                <Modal isOpen={isAddModalOpen} onRequestClose={() => setIsAddModalOpen(false)}>
                    ...
                    />
                    <button className='button confirm-user' onClick={confirmAddUser}>Confirm</button>
                    <button className='button cancel-user' onClick={() => setIsAddModalOpen(false)}>Cancel</button>
                </Modal>
            )}
        </div>
    );
